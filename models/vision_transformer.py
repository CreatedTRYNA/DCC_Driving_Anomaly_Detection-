...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        mlp_dim = round(mlp_factor * in_feature_dim)  # 多层感知机中隐藏层的神经元个数
        self.mlp = nn.Sequential(OrderedDict([
            ('fc1', nn.Linear(in_feature_dim, mlp_dim)),
            ('act', act()),
            ('dropout', nn.Dropout(mlp_dropout)),
            ('fc2', nn.Linear(mlp_dim, out_feature_dim)),
        ]))
...
class VisionTransformer2D(nn.Module):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, embed_dim=feature_dim)  # 图片patch映射层
        # 例如输入图片大小为224x224，将图片分为固定大小的patch，
        # patch大小为16x16，则每张图像会生成224x224/16x16=196个patch，即输入序列长度为196
        self.num_patches = np.prod(
            [x // y for x, y in zip(input_size, patch_size)]) + 1
        self.cls_token = nn.Parameter(torch.zeros([feature_dim]))  # cls特殊类别字符
        self.pos_embed = nn.Parameter(torch.zeros([self.num_patches, feature_dim]))  # 位置编码

        self.blocks = nn.ModuleList([
            TransformerEncoderLayer(
                in_feature_dim=feature_dim, out_feature_dim=feature_dim, qkv_dim=feature_dim, num_heads=num_heads,
...
class TransformerFusedBlock(nn.Module):
    def __init__(self,
                 feature_dim: int = 768,
                 out_feature_dim: int = 768,
                 num_heads: int = 12,
        ...
        self.blocks = nn.ModuleList([
            TransformerDecoderLayer(
                in_feature_dim=feature_dim, out_feature_dim=out_feature_dim, qkv_dim=feature_dim, num_heads=num_heads,
                mlp_factor=mlp_factor, act=act
            ) for _ in range(num_layers)
        ])
...
class TransformerConcatFusedBlock(nn.Module):
    def __init__(self,
                 feature_dim: int = 768,
                 out_feature_dim: int = 768,
                 num_heads: int = 12,
        ...
        self.mlp = nn.Linear(in_features=feature_dim,out_features=out_feature_dim)
