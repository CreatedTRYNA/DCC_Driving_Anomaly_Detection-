    ...
    if subset == 'train' and type == 'normal':
        ...
        for train_folder in train_folder_list:
            ...
            for normal_video in normal_video_list:
                ...
                for i in range(0, n_frames, sample_duration):  # sample_duration = 32
                    sample_ = sample.copy()
                    sample_['frame_indices'] = list(range(i, min(n_frames, i + sample_duration)))
                    if len(sample_['frame_indices']) < sample_duration:
                        for j in range(sample_duration - len(sample_['frame_indices'])):  # 如果clip小于32帧，则将最后视频的最后一帧重复叠加到clip使其达到32帧
        ...
        for train_folder in train_folder_list:
            ...
            for anormal_video in anormal_video_list:
                ...
                n_frames = len(os.listdir(video_path))
                if n_frames <= 0:
                    print(f"Path {video_path} doesn't contain any data")
                    continue
                sample = {
                    'video': video_path,
                    'label': 0,
    ...
    def __getitem__(self, index):
        if self.subset == 'train':
            ...
            if self.temporal_transform is not None:
                frame_indices = self.temporal_transform(frame_indices)  # 从32帧中取16帧，每隔两帧取一帧

            clip = self.loader(video_path, frame_indices)
            self.spatial_transform.randomize_parameters()
            clip = [self.spatial_transform(img.copy()) for img in clip]
            clip = torch.stack(clip, 0)
            clip = clip.permute(1, 0, 2, 3)  # data with shape (channels, timesteps, height, width) -> "L"
